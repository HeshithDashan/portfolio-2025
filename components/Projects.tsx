...
const Projects = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        // ඔයාගේ Repos ටික අලුත් එකේ ඉඳන් පරණ එකට පිළිවෙලට ගන්නවා
        ...
        // Fork කරපු ඒවා අයින් කරලා, ඔයාගේම ඒවා (Original) විතරක් ගන්නවා
        const myProjects = data.filter((repo: any) => !repo.fork).slice(0, 6); // දැනට උපරිම 6යි පෙන්නන්නෙ
        setProjects(myProjects);
      
      } catch (error) {
        console.log("⚠️ GitHub API Error. Using Backup Projects...");
        
        // API අවුල් ගියොත් පෙන්නන Manual List එක
        setProjects([
          {
            id: 1,
            name: "MR Companion",
